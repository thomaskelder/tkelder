...
goData.noz = pathwayGO(mappingFile = mappingFile, onto = onto)
result.noz <- getSigGroups(goData.noz, 
	new("classicCount", testStatistic = GOFisherTest, name = "Fisher test - classic"))
# Modify the stats to contain annotated pathway counts
for(t in names(result.noz@score)) {
	result.noz@score[[t]] = length(genesInTerm(goData.noz, whichGO = t)[[1]])
}
prefix = paste(output, "pwgo.noz", sep="")
printGraph(goData.noz, result.noz, firstSigNodes = length(result.noz@score), 
	fn.prefix = prefix, useInfo = "all")

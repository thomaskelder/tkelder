...
write(positives(total), "combined_zscores_anov0.05_pos.txt", path)
...
save(results.p, results.pg, results.g, file=paste(outDir, "results.Rd", sep=""))

##### Comparison of 3 GO approaches
#load(paste(outDir, "results.Rd", sep=""))
...
#	}
	...
	png(file = paste(outDir, "venn-pathway.png", sep=""), width = 1200, height = 1000)
	createVenn(goData.p, terms = vennTerms, main = "GO to pathway mappings", sig = FALSE)
	dev.off()
	
	png(file = paste(outDir, "venn-pathway-p-anova.png", sep=""), width = 1200, height = 1000)
	createVenn(goData.p, terms = vennTerms, main = "GO to pathway mappings", sig = TRUE)
	dev.off()

	png(file = paste(outDir, "venn-pathway-pg.png", sep=""), width = 1200, height = 1000)
	createVenn(goData.pg, terms = vennTerms, main = "GO to gene mappings (genes based on pathways)", sig = FALSE)
	dev.off()
	
	png(file = paste(outDir, "venn-pathway-pg-anova.png", sep=""), width = 1200, height = 1000)
	createVenn(goData.pg, terms = vennTerms, main = "GO to gene mappings (genes based on pathways)", sig = TRUE)
	dev.off()

	png(file = paste(outDir, "venn-pathway-g.png", sep=""), width = 1200, height = 1000)
	createVenn(goData.g, terms = vennTerms, main = "GO to gene mappings (all)", sig = FALSE)
	dev.off()
	
	png(file = paste(outDir, "venn-pathway-g-anova.png", sep=""), width = 1200, height = 1000)
	createVenn(goData.g, terms = vennTerms, main = "GO to gene mappings (p < 0.05)", sig = TRUE)
	dev.off()
}
